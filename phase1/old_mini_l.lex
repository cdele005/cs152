...
LOGIC ({AND}|{OR}) 
BOOL  ({TRUE}|{FALSE})
EXPRESS	({L_PAREN}{WSPACE}*)*({DIGIT}+|{IDENT}){WSPACE}*({OPERATE}{WSPACE}*(({DIGIT}+{R_PAREN}*{WSPACE}*)|({IDENT}{WSPACE}*{R_PAREN}*{WSPACE}*)|(({L_PAREN}{WSPACE}*)*({DIGIT}+|{IDENT}|({R_PAREN}))){WSPACE}*))*{R_PAREN}*
...
{READ}{WSPACE}+{IDENT}({COMMA}{WSPACE}*{IDENT})*{WSPACE}*{SEMICOLON}	{printf("READ\nIDENT ");
					...
					printf("\nSEMICOLON\n");}
{IDENT}{WSPACE}*{ASSIGN}{WSPACE}*({DIGIT}+|{EXPRESS}|{IDENT}){WSPACE}*{SEMICOLON}	{
                                                        id_start = 0;
							for(i = 0; i < yyleng; i++){
							if (yytext[i] == ';') {printf("\nSEMICOLON\n");}
